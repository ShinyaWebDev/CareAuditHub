...
export async function getApiSyncOverview() {
  const connections = await db.select().from(apiSyncConnections).orderBy(apiSyncConnections.id)
  const history = await db.select().from(apiSyncHistory).orderBy(desc(apiSyncHistory.timestamp))
...
export async function retryApiSyncConnection(id: string) {
  const [connection] = await db.select().from(apiSyncConnections).where(eq(apiSyncConnections.id, id)).limit(1)

  if (!connection) {
    throw notFound('API sync connection', id)
  }
